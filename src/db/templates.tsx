...
export const createTemplate = async (template: Omit<IEmailTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<IEmailTemplate> => {
  ...
  const docRef = await addDoc(collection(db, COLLECTION_NAME), templateWithDates);
  return { ...templateWithDates, id: docRef.id,createdAt: now.toDate(),updatedAt: now.toDate() } as IEmailTemplate;
};
