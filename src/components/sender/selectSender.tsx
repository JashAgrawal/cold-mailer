...
const SelectSender = () => {
  ...
  return (
    <>
     <Select value={selectedSender?.id} onValueChange={(value) => {
        if(value === 'create') {
            setOpen(true);
            return;
        }
      ...
      if (selectedSender) {
        ...
        <SelectContent>
            ...
            {isLoading && <SelectItem disabled value='loading'>Loading...</SelectItem>}
            {senders?.length === 0 && <SelectItem disabled value='no'>No senders found</SelectItem>}
